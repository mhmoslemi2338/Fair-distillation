...
def plot_gradient_distribution(
    ...
    # Shared bins for all groups
    bin_edges = np.linspace(xlim[0], xlim[1], bins + 1)
    bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])

    # ---- Real subgroups ----
    for group_id, grads_list in group_grads.items():
        flat = torch.cat([g.flatten() for g in grads_list]) \
                    .detach().cpu().numpy()

        hist, _ = np.histogram(flat, bins=bin_edges, density=False)

        # max-normalize (shape only)
        # hist = hist / hist.max() if hist.max() > 0 else hist

        plt.plot(
            bin_centers,
            hist,
            label=f"Real Group {group_id}",
            alpha=0.9
        )

    # ---- Synthetic gradients ----
    if synthetic_grads is not None:
        flat_syn = torch.cat([g.flatten() for g in synthetic_grads]) \
                         .detach().cpu().numpy()

        hist_syn, _ = np.histogram(flat_syn, bins=bin_edges, density=False)
        # hist_syn = hist_syn / hist_syn.max() if hist_syn.max() > 0 else hist_syn
    ...
    title = f"Gradient Distribution (Histogram) at Iter {iter_idx}"
    ...
    if class_idx is not None:
        filename = os.path.join(save_path, f'grad_hist_iter{iter_idx}_class{class_idx}.png')
    else:
        filename = os.path.join(save_path, f'grad_hist_iter{iter_idx}.png')

    plt.savefig(filename)
    plt.close()
    print(f"Saved histogram gradient plot to {filename}")
...
def plot_grad_ecdf(group_grads, synthetic_grads, iter_idx, save_path, class_idx=None, xlim=(-0.1,0.1)):
    plt.figure(figsize=(10,6))
    ...
    for gid, grads_list in group_grads.items():
        flat = torch.cat([g.flatten() for g in grads_list]).detach().cpu().numpy()
        flat = flat[(flat >= xlim[0]) & (flat <= xlim[1])]  # optional focus
        x, y = ecdf(flat)
        plt.plot(x, y, label=f"Real Group {gid}", alpha=0.9)

    if synthetic_grads is not None:
        flat = torch.cat([g.flatten() for g in synthetic_grads]).detach().cpu().numpy()
        flat = flat[(flat >= xlim[0]) & (flat <= xlim[1])]
        x, y = ecdf(flat)
        plt.plot(x, y, '--', color='black', linewidth=2, label="Synthetic Data")

    plt.xlim(*xlim)
    plt.ylim(0, 1)
    title = f"Gradient ECDF at Iter {iter_idx}" + (f" | Class {class_idx}" if class_idx is not None else "")
    plt.title(title)
    plt.xlabel("Gradient Value")
    plt.ylabel("ECDF")
    plt.grid(True, alpha=0.3)
    ...
    fn = os.path.join(save_path, f"grad_ecdf_iter{iter_idx}" + (f"_class{class_idx}" if class_idx is not None else "") + ".png")
    plt.savefig(fn)
    plt.close()
...
def main():
    ...
    parser.add_argument('--batch_real', type=int, default=1048, help='batch size for real data')
    parser.add_argument('--batch_train', type=int, default=1048, help='batch size for training networks')
    ...
    for exp in range(args.num_exp):
        ...
        for jj, NAME in enumerate(['DC', 'FairDD', 'BaryFair']):
        # ''' initialize the synthetic data '''
            if jj ==0:
                save_path_tmp = '/home/mmoslem3/scratch/FairDD/results/DC-NoFair/DC_Colored_FashionMNIST_background_ipc50/res_DC_Colored_FashionMNIST_background_ConvNet_50ipc.pt'
            elif jj==1:
                save_path_tmp = '/home/mmoslem3/scratch/FairDD/results/DC-FairDD/FairDD_DC_Colored_FashionMNIST_background_ipc50/res_DC_Colored_FashionMNIST_background_ConvNet_50ip700.pt'
            else:
                save_path_tmp = '/home/mmoslem3/scratch/FairDD/results/DC-NoOrtho/Fair_NoOrtho_DC_Colored_FashionMNIST_background_ipc50/res_DC_Colored_FashionMNIST_background_ConvNet_50ip1000.pt'
            ...
            # image_syn = image_syn.float().detach().requires_grad_(Fal)
                ...
                for ol in range(1):
                    ...
                    for c in range(num_classes):
                        ...
                        # if False: #exp1
                            ...
                            # ---------------------------------------------------------
                            ### <--- NEW PLOTTING CODE HERE 
                            # Plot only occasionally (e.g., every 100 iters) to save time
                            if c == 0:  # c==0 ensures we only plot for the first class in the loop
                                plot_gradient_distribution(group_grads, gw_syn, str(it) + '_'+ NAME, '/home/mmoslem3/scratch/FairDD/')
                                plot_grad_ecdf(group_grads, gw_syn, str(it) + '_' + NAME, '/home/mmoslem3/scratch/FairDD/')
                            # ---------------------------------------------------------
