...
#         return [
#             file1,
#             file1.replace("1000", "700"),
#             file1.replace("1000", "600"),
#             file1.replace("1000", "550"),
#         ]
...
#     """
#     Computes mean gradient vector on whole loader and per sensitive group (a).
#     Returns:
#       v_all: torch vector
#       v_g: dict[gid -> torch vector]
#       n_g: dict[gid -> count]
#     """
#     model.eval()
...
#         # per-group grads: recompute on subset (safe)
...
#     """
#     d[gid] = ||g_all - g_gid||_2, optionally normalized by ||g_all||_2
#     Returns:
#       gids_sorted: list[int]
#       dists: np.ndarray aligned to gids_sorted
#       all_norm: float
#     """
#     if param_mode == "all":
...
#     """
#     Align current distances (gids_cur, d_cur) to reference gids_ref.
#     Missing gid gets NaN.
#     """
#     m = {int(g): float(v) for g, v in zip(gids_cur, d_cur)}
#     out = np.array([m.get(int(g), np.nan) for g in gids_ref], dtype=np.float64)
#     return out
...
# # Plot
# # ----------------------------
# def plot_dispersion_4bars_per_group(dists_mean, dists_std, gids, dataset, ipc, out_dir,
...
#     title = f"{dataset} | ipc={ipc} | per-group gradient dispersion"
...
#     fn = os.path.join(out_dir, f"dispersion_groups_{dataset}_ipc{ipc}_{param_mode}_{normalize}.png")
#     plt.tight_layout()
...
#     # build full real tensors (train set)
#     images_all = [torch.unsqueeze(dst_train[i][0], dim=0) for i in range(len(dst_train))]
#     labels_all = [dst_train[i][1] for i in range(len(dst_train))]
#     images_all = torch.cat(images_all, dim=0).to(args.device)
#     labels_all = torch.tensor(labels_all, dtype=torch.long, device=args.device)

#     # infer groups from train data if present
...
#     # store aligned distances per eval seed
#     per_eval_dists = {k: [] for k in order}
#     gids_ref = None

#     # optional debug: store g_all norms
...
#         # compute distances, align to reference gids
#         for k in order:
...
#     # aggregate
#     dists_mean = {}
#     dists_std = {} if args.num_eval > 1 else None
#     for k in order:
#         arr = np.stack(per_eval_dists[k], axis=0)  # [num_eval, num_groups]
...
#     plot_dispersion_4bars_per_group(
...
#     # print g_all norms (useful if normalize=by_all_norm)
#     for k in order:
#         if len(norms[k]) > 0:
